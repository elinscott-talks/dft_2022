...
\begin{frame}{Koopmans functionals: theory}
   ...
   \begin{overlayarea}{\textwidth}{0.5\paperheight}
      ...
      \only<5->{
         ...
         \begin{itemize}
            \item<6-> orbital density dependence (can use MLWFs)
            \item<7-> screening
         \end{itemize}
      }
      \only<6>{
...
\begin{frame}{\small Koopmans functionals: the workflow(s)}
   ...
   \onslide<4->{(c) via machine learning (resulting band gaps accurate to $\sim$ 0.02 eV)} \blfootcite{DeGennaro2022,Colonna2022,Schubert2022}
   % \onslide<6>{
